...
def PositionalEmbedding(input):
    ...
    return (input + positional_embedding).unsqueeze(-1)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        keys    = keys.reshape(batch_k, key_len, self.heads, self.qk_head_dim)
        values  = values.reshape(batch_k, value_len, self.heads, self.v_head_dim)
